...
export const checkUserPermissions = async (listId, currentUser, requiredRoles, dbName = 'List') => {
    const foundList = await db[dbName].findOne({ _id: listId }).catch(error => { throw error });
    if (!foundList) {
